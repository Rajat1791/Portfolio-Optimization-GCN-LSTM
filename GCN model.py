...
from scipy.optimize import minimize --- FUNCTIONS ---
...
# --- MAIN EXECUTION ---
# Due to length, the rest of this file follows the same training, plotting, evaluation, and portfolio code as GCN-LSTM but using only GCNModel.

# --- MAIN EXECUTION CONTINUED FOR GCN ONLY MODEL ---
...
returns_df_train = price_df.loc[:str(train_end_date)].pct_change().dropna()
returns_df_test = price_df.loc[str(test_start_date):].pct_change().dropna()
...
# GCN model setup
model = GCNModel(input_features=gcn_data.x.shape[1], hidden_channels=64, output_features=1)
...
for epoch in range(100):
    optimizer.zero_grad()
    output = model(train_x, gcn_data.edge_index).flatten()
    ...
    model.eval()
    with torch.no_grad():
        val_output = model(val_x, gcn_data.edge_index).flatten()
...
model.eval()
with torch.no_grad():
    predictions = model(gcn_data.x, gcn_data.edge_index).flatten().numpy()
true_values = gcn_data.y.numpy()

# Plots
plt.figure(figsize=(10, 6))
plt.plot(true_values, label="True Prices", color='blue')
plt.plot(predictions, label="GCN Predicted", color='green', linestyle='--')
plt.title("GCN Stock Price Prediction")
plt.xlabel("Time")
plt.ylabel("Price")
plt.legend()
plt.show()
